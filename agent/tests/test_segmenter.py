...
def observation(offset_seconds: int, kind: str, application_name: str | None = None, file_name: str | None = None) -> Observation:
    return Observation(
...
class SegmentBuilderTests(unittest.TestCase):
    def test_emits_application_and_idle_segments_when_state_changes(self) -> None:
        ...
        self.assertEqual(builder.observe(observation(0, "APPLICATION", "AutoCAD", "ABC_A_Block.dwg")), [])
        self.assertEqual(builder.observe(observation(2, "APPLICATION", "AutoCAD", "ABC_A_Block.dwg")), [])
        ...
        self.assertEqual([(segment.type, segment.application_name, segment.file_name, segment.duration_seconds) for segment in autocad + chrome + idle], [
            ("APPLICATION", "AutoCAD", "ABC_A_Block.dwg", 4),
            ("APPLICATION", "Chrome", None, 4),
            ("IDLE", None, None, 4),
        ])

    def test_flushes_unchanged_activity_every_five_minutes_and_continues_tracking(self) -> None:
        builder = SegmentBuilder(max_segment_seconds=300)

        builder.observe(observation(0, "APPLICATION", "AutoCAD", "ABC_A_Block.dwg"))
        flushed = builder.observe(observation(300, "APPLICATION", "AutoCAD", "ABC_A_Block.dwg"))
        trailing = builder.finish(BASE_TIME + timedelta(seconds=302))

        self.assertEqual([segment.duration_seconds for segment in flushed + trailing], [300, 2])
        self.assertEqual([segment.file_name for segment in flushed + trailing], ["ABC_A_Block.dwg", "ABC_A_Block.dwg"])
    ...
    def test_skip_closes_activity_without_recording_the_excluded_period(self) -> None:
        ...
        self.assertEqual([(segment.type, segment.duration_seconds) for segment in closed], [("APPLICATION", 10)])
