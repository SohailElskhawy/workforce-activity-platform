...
class SimulatorCollectorTests(unittest.TestCase):
    def test_emits_the_repeatable_demo_sequence(self) -> None:
        ...
        observations = list(
            SimulatorCollector(start_at=start_at, step_seconds=5).observations()
        )

        self.assertEqual(
            [
                (item.kind, item.application_name, item.file_name)
        ...
        self.assertEqual(
            [int((item.at - start_at).total_seconds()) for item in observations],
            [0, 5, 10, 15, 20],
        )
