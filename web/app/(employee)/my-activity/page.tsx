...
export default async function MyActivityPage() {
  ...
  return <main className="flex-1 space-y-6 p-6 md:p-10"><ActivityPoller /><PageHeading description="Your captured application activity for today." title="My Activity" /><section className="grid gap-4 sm:grid-cols-3"><Metric label="Active" value={formatDurationFromSeconds(summary.activeSeconds)} /><Metric label="Idle" value={formatDurationFromSeconds(summary.idleSeconds)} /><Metric label="Manual time" value={formatDurationFromMinutes(summary.manualMinutes)} /></section><ApplicationBreakdown applications={summary.applications} /><ActivityTimeline activities={summary.timeline} /></main>;
}
