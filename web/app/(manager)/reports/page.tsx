...
export default async function ReportsPage() {
  ...
  const [metrics, employees, projects, tasks] = await Promise.all([
  ...
  return (
    <main className="flex-1 space-y-6 p-6 md:p-10">
      <PageHeading
        description="Company-level activity metrics. Manual time and automated activity are intentionally reported separately."
        title="Reports"
      />
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Employees" value={String(metrics.employeeCount)} />
        <Metric
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filter the report target and date without interpreting activity as a
            productivity score.
          </CardDescription>
        </CardHeader>
        <CardContent>
