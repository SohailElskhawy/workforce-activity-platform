...
export function demoFixtureCompanyWhere() {
  return { demoFixtureSet: DEMO_FIXTURE_SET };
}

export function buildDemoFixturePlan(_now: Date) {
  return {
    companyCount: 2,
    projectStatuses: ["PLANNED", "ACTIVE", "ON_HOLD", "COMPLETED", "ARCHIVED"],
    taskStatuses: [
      "TODO",
      "IN_PROGRESS",
      "BLOCKED",
...
export async function resetDemoFixtures(): Promise<DemoFixtureResult> {
  ...
  await prisma.company.deleteMany({ where: demoFixtureCompanyWhere() });

  // ---------------------------------------------------------------- company
  const company = await prisma.company.create({
    data: {
      name: COMPANY_NAME,
      demoFixtureSet: DEMO_FIXTURE_SET,
      demoFixtureVersion: DEMO_FIXTURE_VERSION,
    },
  });
  ...
  await prisma.device.create({
    data: {
      companyId: company.id,
      employeeId: can.id,
      deviceId: "WS-IST-0098",
      name: "Can Archived Workstation",
      agentTokenHash: agentTokenHashAyse,
      agentVersion: "0.9.8",
      lastSeenAt: at(18, 16),
      isActive: false,
  ...
  const secondaryCompany = await prisma.company.create({
    data: {
      name: "WorkLens Demo Studio",
      demoFixtureSet: DEMO_FIXTURE_SET,
      demoFixtureVersion: DEMO_FIXTURE_VERSION,
      trackingSettings: {
        create: { idleThresholdSeconds: 420, configVersion: 2 },
      },
    },
  });
  const secondaryManager = await prisma.employee.create({
    data: {
      companyId: secondaryCompany.id,
      firstName: "Selin",
      lastName: "Kurt",
      email: "selin@worklens.demo",
  ...
  const secondaryUser = await prisma.user.create({
    data: {
      companyId: secondaryCompany.id,
      employeeId: secondaryManager.id,
      email: "selin@worklens.demo",
      passwordHash,
      role: "MANAGER",
    },
  ...
  const completedProject = await prisma.project.create({
    data: {
      companyId: company.id,
      name: "Cedar Campus Lighting Retrofit",
      code: "CCL-25",
      clientName: "Cedar Campus",
      status: "COMPLETED",
      startDate: at(180, 9),
      endDate: at(30, 18),
      estimatedHours: 280,
  ...
  const archivedProject = await prisma.project.create({
    data: {
      companyId: company.id,
      name: "Legacy Warehouse Assessment",
      code: "LWA-24",
      clientName: "Legacy Warehouse",
      status: "ARCHIVED",
      startDate: at(700, 9),
      endDate: at(500, 18),
      estimatedHours: 96,
    ...
    data: [
      {
        ...
        title: "Client handover package",
        description: "Completed handover documentation for the retrofit.",
        createdById: managerUser.id,
        status: "COMPLETED",
        priority: "LOW",
        estimatedMinutes: 360,
        dueDate: at(30, 18),
        completedAt: at(31, 16),
      },
      {
        companyId: company.id,
        projectId: archivedProject.id,
        title: "Superseded feasibility option",
        description: "Cancelled after the client changed the project scope.",
        createdById: managerUser.id,
        status: "CANCELLED",
        priority: "LOW",
  ...
  await prisma.employeeLeave.createMany({
    data: [
      ...
      },
      {
        ...
        endDate: at(12, 18),
        days: 1,
        externalId: "demo-leave-1",
      },
    ],
  });
  await prisma.integration.createMany({
    data: [
      { companyId: company.id, provider: "CLICKUP", status: "CONFIGURED", config: { demo: true, workspace: "Demo Design Workspace" } },
      { companyId: company.id, provider: "CLOCKIFY", status: "CONNECTED", config: { demo: true, workspace: "Demo Time Workspace" }, lastSyncAt: at(1, 17) },
      { companyId: company.id, provider: "KOLAY_IK", status: "ERROR", config: { demo: true }, lastError: "Demo connection requires configuration." },
      { companyId: secondaryCompany.id, provider: "CLICKUP", status: "DISABLED", config: { demo: true } },
      { companyId: secondaryCompany.id, provider: "CLOCKIFY", status: "NOT_CONFIGURED" },
      { companyId: secondaryCompany.id, provider: "KOLAY_IK", status: "SYNCING", config: { demo: true } },
    ],
  });
  await prisma.integrationMapping.createMany({
    data: [
      { companyId: company.id, provider: "CLICKUP", externalEntityType: "task", externalId: "demo-clickup-100", internalEntityType: "Task", internalId: taskA.id, metadata: { demo: true } },
      { companyId: company.id, provider: "CLOCKIFY", externalEntityType: "project", externalId: "demo-clockify-200", internalEntityType: "Project", internalId: projectABC.id, metadata: { demo: true } },
    ],
  });
  await prisma.anomaly.createMany({
    data: [
      { companyId: company.id, employeeId: mehmet.id, projectId: projectABC.id, taskId: taskA.id, type: "TIME_DISCREPANCY", severity: "HIGH", status: "OPEN", title: "Manual and tracked time differ", description: "Review the recorded time difference.", fingerprint: "demo-time-open", detectedAt: at(1, 18), periodStart: at(1, 9), periodEnd: at(1, 18), metadata: { differenceMinutes: 130 } },
      { companyId: company.id, employeeId: ayse.id, projectId: projectABC.id, taskId: taskB.id, type: "PROJECT_MISMATCH", severity: "MEDIUM", status: "ACKNOWLEDGED", title: "Activity mapped to another project", description: "Review the project association.", fingerprint: "demo-project-ack", detectedAt: at(2, 18), periodStart: at(2, 9), periodEnd: at(2, 18), metadata: { overlapSeconds: 3600 } },
      { companyId: company.id, employeeId: can.id, projectId: projectMP.id, type: "IDLE_SPIKE", severity: "LOW", status: "RESOLVED", title: "Extended idle period", description: "Review the activity pattern.", fingerprint: "demo-idle-resolved", detectedAt: at(3, 18), periodStart: at(3, 9), periodEnd: at(3, 18), resolvedAt: at(3, 19), resolvedById: managerUser.id, resolutionNotes: "Approved training session." },
      { companyId: company.id, employeeId: elif.id, projectId: projectMP.id, taskId: taskChiller.id, type: "TIME_DISCREPANCY", severity: "MEDIUM", status: "DISMISSED", title: "Time entry needs context", description: "Review the reported time.", fingerprint: "demo-time-dismissed", detectedAt: at(4, 18), periodStart: at(4, 9), periodEnd: at(4, 18), resolvedAt: at(4, 19), resolvedById: managerUser.id, resolutionNotes: "Client meeting was recorded separately." },
    ],
  });
  await prisma.auditLog.createMany({
    data: [
      { companyId: company.id, actorUserId: managerUser.id, action: "DEMO_FIXTURE_SEEDED", entityType: "Company", entityId: company.id, metadata: { count: activityCount } },
      { companyId: company.id, actorUserId: managerUser.id, action: "PROJECT_UPDATED", entityType: "Project", entityId: projectABC.id, metadata: { status: "ACTIVE" } },
    ],
  });
  await prisma.systemSettings.upsert({
    where: { id: "system" },
    create: { notificationDueSoonHours: 48, defaultIdleThresholdSeconds: 300 },
    update: { notificationDueSoonHours: 48, defaultIdleThresholdSeconds: 300 },
  });
  ...
  return {
    ...
    employees: 9,
    projects: 6,
    tasks: taskSeeds.length + 2,
    activities: activityCount,
    anomalies: 4,
  };
}

export async function seedDemoFixtures(): Promise<DemoFixtureResult> {
  return resetDemoFixtures();
}

export async function disconnectDemoFixtureClient() {
