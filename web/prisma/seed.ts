...
async function main() {
    ...
    const [deptElectrical, deptMechanical, deptCivil, deptPM] =
        await Promise.all([
    ...
    const projectMP = await prisma.project.create({
        data: {
            ...
            code: "MP-HVAC",
            description: "HVAC system replacement and capacity upgrade for Metro Plaza office tower.",
    ...
    const projectST = await prisma.project.create({
        data: {
            ...
            code: "ST-STR",
            description: "Independent structural review of the Seaside Tower residence project.",
    ...
    const taskSeeds: TaskSeed[] = [
        {
            ...
            description: "Complete electrical drawing set for A Block: power plans, riser diagram, panel schedules.",
        },
        {
            ...
            description: "Electrical drawing set for B Block, based on approved A Block template.",
        },
        {
            ...
            description: "Revise lighting layouts per client comments from the second design review.",
        },
        {
        ...
        },
        {
            ...
            description: "Route main cable trays through parking levels and coordinate with mechanical.",
        },
        {
        ...
        },
        {
            ...
            description: "Ductwork routing for floors 3-7 with new AHU configuration.",
        },
        {
            ...
            description: "Blocked: waiting for as-built cooling load report from facility management.",
        },
        {
            ...
            dueInDays: 30,
            estimatedMinutes: 480,
            description: "Commissioning sequence and test plan for ventilation systems.",
        },
        {
            ...
            description: "Independent verification of vertical and lateral load paths.",
        },
        {
    ...
    const timeEntrySeed: Array<{
        ...
        notes: string;
            ...
            { employeeId: mehmet.id, projectId: projectABC.id, taskId: taskA.id, daysAgo: 8, startHour: 9, durationMinutes: 150, notes: "A Block power plan drafting" },
            { employeeId: mehmet.id, projectId: projectABC.id, taskId: taskA.id, daysAgo: 6, startHour: 13, durationMinutes: 120, notes: "Riser diagram updates" },
            { employeeId: mehmet.id, projectId: projectABC.id, taskId: taskLighting.id, daysAgo: 4, startHour: 10, durationMinutes: 90, notes: "Lighting revision per client comments" },
            { employeeId: mehmet.id, projectId: projectABC.id, taskId: taskPanel.id, daysAgo: 2, startHour: 14, durationMinutes: 180, notes: "Panel schedule recalculation" },
            { employeeId: ayse.id, projectId: projectABC.id, taskId: taskB.id, daysAgo: 7, startHour: 9, durationMinutes: 120, notes: "B Block template setup" },
            { employeeId: ayse.id, projectId: projectABC.id, taskId: taskB.id, daysAgo: 5, startHour: 11, durationMinutes: 150, notes: "B Block power plan drafting" },
            { employeeId: ayse.id, projectId: projectABC.id, taskId: taskTray.id, daysAgo: 3, startHour: 9, durationMinutes: 60, notes: "Cable tray routing study" },
            { employeeId: ayse.id, projectId: projectABC.id, taskId: taskTray.id, daysAgo: 1, startHour: 15, durationMinutes: 90, notes: "Parking level coordination" },
            { employeeId: can.id, projectId: projectMP.id, taskId: taskDuct.id, daysAgo: 8, startHour: 10, durationMinutes: 120, notes: "Ductwork layout floors 3-5" },
            { employeeId: can.id, projectId: projectMP.id, taskId: taskDuct.id, daysAgo: 6, startHour: 14, durationMinutes: 90, notes: "Ductwork layout floors 6-7" },
            { employeeId: can.id, projectId: projectMP.id, taskId: taskAHU.id, daysAgo: 4, startHour: 9, durationMinutes: 60, notes: "Vendor proposal comparison" },
            { employeeId: can.id, projectId: projectMP.id, taskId: taskDuct.id, daysAgo: 2, startHour: 13, durationMinutes: 150, notes: "Shaft coordination revisions" },
            { employeeId: elif.id, projectId: projectMP.id, taskId: taskAHU.id, daysAgo: 7, startHour: 9, durationMinutes: 180, notes: "AHU selection review meeting prep" },
            { employeeId: elif.id, projectId: projectMP.id, taskId: taskChiller.id, daysAgo: 5, startHour: 11, durationMinutes: 60, notes: "Chiller capacity preliminary check" },
            { employeeId: elif.id, projectId: projectMP.id, taskId: taskChiller.id, daysAgo: 3, startHour: 14, durationMinutes: 90, notes: "Waiting on as-built load report" },
            { employeeId: elif.id, projectId: projectMP.id, taskId: taskAHU.id, daysAgo: 1, startHour: 10, durationMinutes: 120, notes: "Selection review follow-ups" },
            { employeeId: burak.id, projectId: projectST.id, taskId: taskLoad.id, daysAgo: 8, startHour: 9, durationMinutes: 150, notes: "Load path model setup" },
            { employeeId: burak.id, projectId: projectST.id, taskId: taskLoad.id, daysAgo: 6, startHour: 13, durationMinutes: 120, notes: "Vertical load verification" },
            { employeeId: burak.id, projectId: projectST.id, taskId: taskFoundation.id, daysAgo: 4, startHour: 10, durationMinutes: 90, notes: "Foundation drawing markup" },
            { employeeId: burak.id, projectId: projectST.id, taskId: taskLoad.id, daysAgo: 2, startHour: 14, durationMinutes: 60, notes: "Lateral system spot checks" },
            { employeeId: zeynep.id, projectId: projectABC.id, taskId: taskPanel.id, daysAgo: 7, startHour: 9, durationMinutes: 60, notes: "Progress tracking update" },
            { employeeId: zeynep.id, projectId: projectMP.id, taskId: taskChiller.id, daysAgo: 5, startHour: 11, durationMinutes: 90, notes: "Blocked item follow-up with FM team" },
            { employeeId: zeynep.id, projectId: projectABC.id, taskId: taskLighting.id, daysAgo: 3, startHour: 13, durationMinutes: 60, notes: "Review coordination" },
            { employeeId: zeynep.id, projectId: projectST.id, taskId: taskLoad.id, daysAgo: 1, startHour: 15, durationMinutes: 120, notes: "Kickoff planning for structural review" },
        ];
    ...
    for (let daysAgo = 1; daysAgo <= 14; daysAgo++) {
        ...
        for (const person of [
            { employee: mehmet, device: deviceMehmet, project: projectABC, task: taskA, code: "ABC-ELE" },
            { employee: ayse, device: deviceAyse, project: projectABC, task: taskB, code: "ABC-ELE" },
        ]) {
            let seq = 0;
            ...
            const add = async (data: {
                ...
                const profile =
                    data.appIndex !== undefined ? appProfiles[data.appIndex % appProfiles.length] : null;
                await prisma.activity.create({
                    data: {
                        ...
                        windowTitle: profile
                            ...
                            : null,
                        fileName: profile?.ext ? `${person.code}_${person.task.title.replace(/\s+/g, "_")}.${profile.ext}` : null,
                        type: data.type,
                    },
                });
                activityCount++;
            ...
            await add({ type: "SYSTEM_START", startHour: 8, startMinute: 30, durationSeconds: 5 });
            await add({ type: "COMPUTER_UNLOCK", startHour: 8, startMinute: 31, durationSeconds: 5 });
            await add({ type: "APPLICATION", startHour: 9, startMinute: 0, durationSeconds: 5400, appIndex: daysAgo });
            await add({ type: "APPLICATION", startHour: 10, startMinute: 45, durationSeconds: 4500, appIndex: daysAgo + 1 });
            await add({ type: "IDLE", startHour: 12, startMinute: 0, durationSeconds: 5400 });
            await add({ type: "APPLICATION", startHour: 13, startMinute: 30, durationSeconds: 12600, appIndex: daysAgo + 2 });
            await add({ type: "COMPUTER_LOCK", startHour: 17, startMinute: 5, durationSeconds: 5 });
            await add({ type: "SYSTEM_STOP", startHour: 17, startMinute: 10, durationSeconds: 5 });
        }
    }
    ...
    console.log(`  Managers:    2 (manager@worklens.demo, manager2@worklens.demo)`);
    console.log(`  Employees:   6 (employee@worklens.demo -> Mehmet Yilmaz)`);
    console.log(`  Projects:    3 (primary: ABC AVM Electrical Project)`);
    console.log(`  Tasks:       ${taskSeeds.length} (primary: A Block Electrical Drawing)`);
